...
numbers <- function (x) { number_format(accuracy = 1,
                                   suffix = "M",
                                   big.mark = ",")(x) }
...
gvWaffle <- ggplot(GvFire, aes(fill = fire, values = acreHndThou)) +
  ...
  scale_x_discrete() +
  scale_fill_manual(values = c("#FFC107", "#601A4A", "#008870"), # mixed, replacement, surface
                    name = "Fire Severity", 
  ...
  scale_y_continuous(labels=numbers) +
...
ggsave(filename="waffle.png",width=10,height=8,units='in',dpi=300)
